...
- **Default agentId** — All MCP calls now tagged with `claude-code` as default agent_id. Configurable via `RADAR_AGENT_ID` in `~/.radar/.env`. Claude can still override per-call via the `agentId` tool parameter.
